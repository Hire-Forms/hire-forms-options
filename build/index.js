...
class Options extends React.Component {
    ...
    componentWillMount() {
        this.setState({ values: sort_1.sortValues(this.props) });
    }
    componentWillReceiveProps(nextProps) {
        this.setState({ values: sort_1.sortValues(nextProps) });
    }
    highlightPrev() {
    ...
    render() {
        ...
        const listitems = this.state.values.map((optionData, index) => React.createElement(option_1.default, __assign({}, this.props, {active: this.state.activeIndex === index, key: index, optionData: optionData, onClick: () => this.setState({ activeIndex: index }, () => this.select())})));
